    ...
    return (
        <>
            <section className='pb-[80px] pt-[31px] px-[20px] md:px-[32px]'>
                ...
                <ul className='flex flex-wrap md:gap-x-[8px] md:gap-y-[32px] gap-[20px]'>
                    {products.products?.map(item => (
                        <li key={item._id} className='flex flex-col gap-[8px] w-[335px] md:w-[229px]'>
                            ...
                            <div className='bg-white rounded-[20px] p-20'>
                                <div className='mb-[17px]'>
                                    <div className='flex justify-between items-center mb-[4px]'>
                                        <h3 className='text-16 leading-140 font-bold text-heading'>{item.name}</h3>
                                        <p className='text-16 leading-140 font-bold text-heading'>৳{item.price}</p>
                                    </div>
            ...
            <Modal isOpen={isModalOpen} onClose={closeModal}>
                {isLoginForm ? <ModalLoginForm openLoginForm={openLoginForm} /> : <ModalRegisterForm openLoginForm={openLoginForm} />}
            </Modal>
        </>
    );
}
